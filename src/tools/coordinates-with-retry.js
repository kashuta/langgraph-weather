import { coordinatesTool } from "./coordinates.js";
  ...
  const retryingFunc = async (input) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (Math.random() < 0.5) {
          throw new Error("Искусственный сбой в coordinates tool!");
        }
...
export const coordinatesToolWithRetry = withRetry(coordinatesTool, {
  maxAttempts: 3,
  initialDelay: 1000,
  backoffMultiplier: 2,
});
